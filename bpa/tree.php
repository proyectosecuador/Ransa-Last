...
$root = $_SERVER['DOCUMENT_ROOT'].'/ransa/archivos/OT/'.$ciudad;
...
if( file_exists($postDir) ) {
	...
	if( count($files) > 2 ) { // The 2 accounts for . and ..
		...
		foreach( $files as $file ) {
			...
			$rpta = ControladorNovedades::ctrConsultarNovedadesEquipos('%'.$htmlName,"rutaot");
			...
			if( file_exists($postDir . $file) && $file != '.' && $file != '..' ) {
				if( is_dir($postDir . $file) && (!$onlyFiles || $onlyFolders) )
					...
					echo "<li class='file ext_{$ext}'>{$checkbox}<a rel='" . $htmlRel . "'>" . $rpta["otnum"]."---".$rpta["fecha_concluida"]."</a></li>";
			}
		}
