	        ...
	        $datosJson = '{
	  		...
	  		for ($i=0; $i < count($rpta) ; $i++) {
								...
								$acciones = "<div class='btn-group'>".$texto.$botonvisualizar.$botondescarga."</div> ";  				
							  			...
							  			$datosJson .= '
						        				    [';
						  				$datosJson .= '"'.date("d-m-Y",strtotime($rpta[$i]["fecha_programada"])).'",
						  							  "'.$rpta[$i]["razonsocial"].'",';
						  				$datosJson .= '"'.$rpta[$i]["descripcion"].'",';
										$datosJson .= '"'.$rpta[$i]["comentarios"].'",';
										$datosJson .= '"'.$rpta[$i]["codigo_generado"].'",';
										$datosJson .= '"'.$rpta[$i]["nguias"].'",';
						  				$datosJson .= '"'.$acciones.'"';
								    	$datosJson .= '],';	
								    	$valorexistente = true;
